...
export function shiftSheetRows(
  ...
  const shift = rowsToInsert;

  // 1. Shift row elements' r attributes.
  const newRowByNum = new Map<number, XmlEl>();
  for (const [rowNum, rowEl] of sheet.rowByNum) {
    if (rowNum >= insertAtRow) {
      rowEl.setAttribute("r", String(rowNum + shift));
      newRowByNum.set(rowNum + shift, rowEl);
    } else {
      newRowByNum.set(rowNum, rowEl);
    }
  }
  sheet.rowByNum.clear();
  for (const [k, v] of newRowByNum) sheet.rowByNum.set(k, v);

  // 2. Shift cell elements' r attributes.
  const newCells = new Map<number, Map<number, CellData>>();
  for (const [rowNum, colMap] of sheet.cells) {
    const newRow = rowNum >= insertAtRow ? rowNum + shift : rowNum;
    const newColMap = new Map<number, CellData>();
    for (const [colNum, cellData] of colMap) {
      const cellEl = sheet.rowByNum.get(newRow)?.childNodes;
      // Update the r attribute on the DOM element.
      if (cellData.ref) {
        // Find the <c> element by scanning row children.
        const rowEl = sheet.rowByNum.get(newRow) || sheet.rowByNum.get(rowNum);
        if (rowEl) {
          for (let i = 0; i < rowEl.childNodes.length; i++) {
            const child = rowEl.childNodes[i];
            if ((child as XmlEl).nodeType === 1) {
              const el = child as XmlEl;
              if (el.localName === "c" || el.nodeName === "c") {
                const r = el.getAttribute("r");
                if (r === cellData.ref) {
                  const newRef = cellData.ref.replace(/(\d+)$/, String(newRow));
                  el.setAttribute("r", newRef);
                  cellData.ref = newRef;
                  break;
                }
              }
      ...
      newColMap.set(colNum, cellData);
    }
    newCells.set(newRow, newColMap);
  }
  sheet.cells.clear();
  for (const [k, v] of newCells) sheet.cells.set(k, v);

  // 3. Shift formulaMap keys.
  ...
  for (const [ref, formula] of sheet.formulaMap) {
    ...
    if (match) {
      ...
      if (rowNum >= insertAtRow) {
        newFormulaMap.set(`${match[1]}${rowNum + shift}`, formula);
      } else {
  ...
  sheet.formulaMap.clear();
  for (const [k, v] of newFormulaMap) sheet.formulaMap.set(k, v);

  // 4. Shift valueMap keys.
  const newValueMap = new Map<string, string>();
  for (const [ref, val] of sheet.valueMap) {
    const match = ref.match(/^([A-Z]+)(\d+)$/);
    if (match) {
      const rowNum = parseInt(match[2]);
      if (rowNum >= insertAtRow) {
        newValueMap.set(`${match[1]}${rowNum + shift}`, val);
      } else {
        newValueMap.set(ref, val);
      }
    } else {
      newValueMap.set(ref, val);
    }
  }
  sheet.valueMap.clear();
  for (const [k, v] of newValueMap) sheet.valueMap.set(k, v);

  // 5. Shift merge ranges.
  const colLetter = (ref: string) => ref.replace(/\d+$/, "");
  const rowNum = (ref: string) => parseInt(ref.replace(/[A-Z]+/, ""));
  for (const merge of sheet.merges) {
    const parts = merge.ref.split(":");
    if (parts.length === 2) {
      const r1 = rowNum(parts[0]);
      const r2 = rowNum(parts[1]);
      if (r1 >= insertAtRow || r2 >= insertAtRow) {
        const newR1 = r1 >= insertAtRow ? r1 + shift : r1;
        const newR2 = r2 >= insertAtRow ? r2 + shift : r2;
        merge.ref = `${colLetter(parts[0])}${newR1}:${colLetter(parts[1])}${newR2}`;
      }
    }
  }
